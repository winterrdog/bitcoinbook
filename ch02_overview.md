...
Alice’s transactionminingoperational overviewid=mining-operational-overview is now propagated on the Bitcoin network. It does not become part of the *blockchain* until it is included in a block by a process called *mining* and that block has been validated by full nodes. See [???](#mining) for a detailed explanation.
